    ...
    <div className="container">
      <div className="row">
        <form onSubmit={(event) => onCitySearch(event)} className="input-group mb-3">
          <input name='cityName' onChange={(e) => {setInput(e.target.value);searchForAutocompleteAndUpdateName()}} type="text" className="form-control" placeholder="Enter Place" aria-label="Enter Place" aria-describedby="Enter Place"></input>
          <button id="SearchBtn" className="input-group-text p-1 btn btn-primary">Search Place</button>
        </form>
        {autocompleteList.map = (a) => {
        }}
      </div>
    </div>
  )
